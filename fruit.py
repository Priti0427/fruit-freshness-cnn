...
def main():
    ...
    if len(samples) == 0:
        raise RuntimeError(f"No PNG images were found inside the fresh/rotten folders under {SOURCE_DIRS}.")

    class_names = ["ripe", "rotten"]
    class_to_idx = {"ripe": 0, "rotten": 1}
    num_classes = 2
    ...
    for class_id, class_name in enumerate(class_names):
        print(f"{class_name}: {np.sum(all_labels == class_id)}")
    ...
    print(f"Training:   {len(train_indices)} ({len(train_indices) / len(samples) * 100:.1f}%)")
    print(f"Validation: {len(val_indices)} ({len(val_indices) / len(samples) * 100:.1f}%)")
    print(f"Test:       {len(test_indices)} ({len(test_indices) / len(samples) * 100:.1f}%)")
    ...
    for class_id, class_name in enumerate(class_names):
        print(f"{class_name}: {np.sum(train_labels == class_id)}")
    ...
    test_accuracy = accuracy_score(all_labels_test, all_predictions)
    ...
    print(classification_report(all_labels_test, all_predictions, target_names=class_names, digits=4))

    cm = confusion_matrix(all_labels_test, all_predictions)
